package red.man10.man10highlow.util.sql;
...
public class SQLManager {
    ...
    public void init(){
        ...
        // ユーザースタッツ
        // ユーザーの最大獲得金額と合計獲得金額
        execute("CREATE TABLE if not exists mhl_user_stats(" +
                "id int auto_increment not null primary key," +
                "time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP," +
                "name VARCHAR(18),"+ // プレイヤー名
                "uuid VARCHAR(40),"+ // UUID
                "max_win BIGINT,"+ // 最大獲得金額
                "total_win BIGINT"+ // 合計獲得金額
                ");");
    }
    ...
    public int countRows(String table) {
        int count = 0;
        ResultSet set = this.query(String.format("SELECT * FROM %s", table)).rs;
